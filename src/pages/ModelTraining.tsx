...
export default function ModelTraining() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0a0a0a] text-brand-primary">
      <div className="p-8 max-w-5xl mx-auto space-y-6">
        <div>
          <h2 className="text-2xl font-bold font-sans text-brand-primary flex items-center gap-2">
            <Play className="w-5 h-5 text-brand-blue-light" /> 模型训练 / 样本投喂
          </h2>
          <p className="text-sm text-brand-muted mt-2">只有管理员可以提交确诊的历史审计样本。模型采用 <span className="text-brand-blue">行业先验权重 + 样本训练权重融合</span> 的机制。</p>
        </div>
        ...
        <div className="bg-[#121212] border border-[var(--bg-card-2)] rounded-lg p-6">
           <h3 className="text-lg font-medium mb-6 flex items-center gap-2 border-b border-[rgba(15,23,42,0.08)] pb-3">
             配置目标模型
           </h3>
           ...
           <div className="grid grid-cols-2 gap-6 mb-6">
             <div>
               ...
               <select 
                 className="w-full bg-brand-deep border border-[rgba(15,23,42,0.14)] rounded px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-[#3AB7FF]"
                 value={industryType}
             ...
             <div>
               ...
               <input 
                 className="w-full bg-brand-deep border border-[rgba(15,23,42,0.14)] rounded px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-brand-blue"
                 value={projectType}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[#121212] border border-[var(--bg-card-2)] rounded-lg p-6 flex flex-col h-full">
            <h3 className="text-lg font-medium mb-4 flex items-center gap-2 border-b border-[rgba(15,23,42,0.14)] pb-3">
              手工输入样本
            </h3>
            
            <div className="space-y-3 flex-1">
              {samples.map((s, i) => (
                <div key={i} className="flex gap-2 items-center bg-[#F7FAFC] border border-[rgba(15,23,42,0.14)] p-2 rounded">
                  <span className="text-xs text-brand-muted w-4">{i+1}.</span>
                  <input type="number" step="0.01" value={s.X1} onChange={e => handleSampleChange(i, 'X1', e.target.value)} className="w-16 bg-white border border-[rgba(15,23,42,0.14)] rounded text-xs px-1 py-1" placeholder="X1" disabled={!isAdmin} />
                  <input type="number" step="0.01" value={s.X2} onChange={e => handleSampleChange(i, 'X2', e.target.value)} className="w-16 bg-white border border-[rgba(15,23,42,0.14)] rounded text-xs px-1 py-1" placeholder="X2" disabled={!isAdmin} />
                  <input type="number" step="0.01" value={s.X3} onChange={e => handleSampleChange(i, 'X3', e.target.value)} className="w-16 bg-white border border-[rgba(15,23,42,0.14)] rounded text-xs px-1 py-1" placeholder="X3" disabled={!isAdmin} />
                  <select value={s.label} onChange={e => handleSampleChange(i, 'label', e.target.value)} className="w-20 bg-white border border-[rgba(15,23,42,0.14)] rounded text-xs px-1 py-1" disabled={!isAdmin}>
            ...
            <button onClick={handleAddSample} className="w-full mt-4 flex items-center justify-center gap-2 border border-dashed border-[rgba(15,23,42,0.14)] bg-white py-2 rounded text-sm text-gray-400 hover:text-gray-200 hover:border-gray-500 transition-colors" disabled={!isAdmin}>
          ...
          <div className="bg-[#121212] border border-[var(--bg-card-2)] rounded-lg p-6">
            ...
            <div className="bg-white border border-dashed border-[rgba(15,23,42,0.14)] rounded p-8 text-center pb-12 relative h-48 flex flex-col justify-center items-center">
              ...
              />
              {csvFile && (
                <div className="mt-4 text-sm text-brand-blue font-medium border border-#005EB8/30 bg-brand-blue/10 px-3 py-1 rounded inline-block">
                  已选择: {csvFile.name}
                </div>
              )}
            </div>
        ...
        {trainingResult && (
          <div className="bg-[#121212] border border-[var(--bg-card-2)] rounded-lg p-6">
            <div className="flex items-center justify-between border-b border-[rgba(15,23,42,0.14)] pb-3 mb-4">
            ...
            <div className="grid grid-cols-3 gap-6">
               <div className="bg-white p-4 rounded border border-[rgba(15,23,42,0.1)]">
               ...
               <div className="bg-white p-4 rounded border border-[rgba(15,23,42,0.1)]">
                 ...
                 <div className="font-mono text-sm space-y-1 text-[#005EB8]">
               ...
               <div className="bg-[#332a1c] p-4 rounded border border-#005EB8/50">
        ...
        {currentWeights && (
          <div className="bg-[#1a1c23] border border-[#2c3345] rounded-lg p-6">
            <h4 className="text-sm font-medium text-[#005EB8] mb-3">当前生效的 {industryType} 模型权重:</h4>
