      ...
      <div className="p-8 max-w-5xl mx-auto space-y-6">
        <div>
          <h2 className="text-2xl font-bold font-sans text-gray-100 flex items-center gap-2">
            <Play className="w-5 h-5 text-[#D4AF37]" /> 模型训练 / 样本投喂
          </h2>
          <p className="text-sm text-gray-400 mt-2">只有管理员可以提交确诊的历史审计样本。模型采用 <span className="text-[#D4AF37]">行业先验权重 + 样本训练权重融合</span> 的机制。</p>
        </div>
        ...
        <div className="bg-[#121212] border border-[#242424] rounded-lg p-6">
           <h3 className="text-lg font-medium mb-6 flex items-center gap-2 border-b border-[#333333] pb-3">
             配置目标模型
           </h3>
           ...
           <div className="grid grid-cols-2 gap-6 mb-6">
             <div>
               ...
               <select 
                 className="w-full bg-[#1A1A1A] border border-[#333333] rounded px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-[#D4AF37]"
                 value={industryType}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[#121212] border border-[#242424] rounded-lg p-6 flex flex-col h-full">
            <h3 className="text-lg font-medium mb-4 flex items-center gap-2 border-b border-[#333333] pb-3">
              手工输入样本
            </h3>
            
            <div className="space-y-3 flex-1">
              {samples.map((s, i) => (
                <div key={i} className="flex gap-2 items-center bg-[#1A1A1A] border border-[#333333] p-2 rounded">
                  <span className="text-xs text-gray-500 w-4">{i+1}.</span>
                  <input type="number" step="0.01" value={s.X1} onChange={e => handleSampleChange(i, 'X1', e.target.value)} className="w-16 bg-[#242424] border border-[#444] rounded text-xs px-1 py-1" placeholder="X1" disabled={!isAdmin} />
                  <input type="number" step="0.01" value={s.X2} onChange={e => handleSampleChange(i, 'X2', e.target.value)} className="w-16 bg-[#242424] border border-[#444] rounded text-xs px-1 py-1" placeholder="X2" disabled={!isAdmin} />
                  <input type="number" step="0.01" value={s.X3} onChange={e => handleSampleChange(i, 'X3', e.target.value)} className="w-16 bg-[#242424] border border-[#444] rounded text-xs px-1 py-1" placeholder="X3" disabled={!isAdmin} />
                  <select value={s.label} onChange={e => handleSampleChange(i, 'label', e.target.value)} className="w-20 bg-[#242424] border border-[#444] rounded text-xs px-1 py-1" disabled={!isAdmin}>
            ...
            <button onClick={handleAddSample} className="w-full mt-4 flex items-center justify-center gap-2 border border-dashed border-[#444] bg-[#1A1A1A] py-2 rounded text-sm text-gray-400 hover:text-gray-200 hover:border-gray-500 transition-colors" disabled={!isAdmin}>
          ...
          <div className="bg-[#121212] border border-[#242424] rounded-lg p-6">
            ...
            <div className="bg-[#1A1A1A] border border-dashed border-[#444] rounded p-8 text-center pb-12 relative h-48 flex flex-col justify-center items-center">
              ...
              />
              {csvFile && (
                <div className="mt-4 text-sm text-[#D4AF37] font-medium border border-[#D4AF37]/30 bg-[#D4AF37]/10 px-3 py-1 rounded inline-block">
                  已选择: {csvFile.name}
                </div>
              )}
            </div>
            ...
            <div className="mt-8 flex justify-end">
              <button 
                ...
                className="bg-[#D4AF37] text-black px-6 py-2.5 rounded font-medium flex items-center gap-2 hover:bg-[#F3CE5E] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        {trainingResult && (
          <div className="bg-[#121212] border border-[#242424] rounded-lg p-6">
            ...
            <div className="grid grid-cols-3 gap-6">
               ...
               <div className="bg-[#332a1c] p-4 rounded border border-[#D4AF37]/50">
                 <div className="text-xs text-[#D4AF37] mb-2 flex justify-between">
