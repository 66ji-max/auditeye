...
export default function ProjectList() {
  ...
  return (
    <div className="h-full w-full bg-brand-bg2 p-4 md:p-6 text-gray-200 overflow-y-auto custom-scrollbar">

      {systemMode === 'demo-readonly' && (
        <div className="bg-brand-card border border-[#3AB7FF]/50 rounded-lg p-4 mb-6 flex flex-col md:flex-row items-start md:items-center justify-between shadow-xl">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex flex-col items-center justify-center p-4">
          <div className="bg-brand-card border border-brand-border-subtle rounded-lg shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-4 border-b border-brand-border-medium shrink-0">
              <h3 className="font-semibold text-lg text-brand-accent">新建审计项目</h3>
            ...
            <div className="p-5 space-y-4 overflow-y-auto">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-brand-bg2 border border-brand-border-medium rounded px-3 py-2 text-sm focus:outline-none focus:border-[#0091DA] text-white"
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-brand-bg2 border border-brand-border-medium rounded px-3 py-2 text-sm focus:outline-none focus:border-[#0091DA] text-gray-200"
                >
              ...
              <div>
                ...
                <div 
                  ...
                  className="w-full bg-brand-bg2 border border-dashed border-brand-border-medium rounded p-4 text-center cursor-pointer hover:border-[#0091DA]/50 transition-colors"
                >
                  {isSubmitting ? (
                    <div className="animate-pulse">
                      <Upload className="w-5 h-5 text-gray-500 mx-auto mb-2 opacity-50" />
                      <div className="text-xs text-brand-accent">文件上传处理中...</div>
                    </div>
                  ) : (
                    <>
                ...
                {files.length > 0 && (
                  <div className="mt-3 space-y-1 max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                    {files.map((f, i) => (
                      <div key={i} className="flex items-center justify-between bg-brand-bg2 border border-brand-border-medium p-2 rounded text-xs">
            ...
            <div className="p-4 border-t border-brand-border-subtle flex justify-end gap-3 bg-brand-bg2 shrink-0">
      ...
      <div className="max-w-7xl mx-auto flex flex-col h-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 shrink-0 gap-4 mt-2">
          ...
          <button 
            ...
            className="w-full md:w-auto px-5 py-2.5 bg-gradient-to-br from-brand-accent to-brand-accent-hover text-white font-medium text-sm rounded-lg shadow-[0_6px_18px_rgba(0,94,184,0.22)] flex items-center justify-center gap-2 transition-all hover:brightness-110 hover:-translate-y-0.5">
        ...
        <div className="flex gap-4 mb-6 shrink-0">
          <div className="relative w-full md:w-80">
            ...
            <input 
              ...
              className="w-full bg-brand-card border border-brand-border-subtle rounded-lg px-9 py-2 text-sm focus:outline-none focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan/30 text-white placeholder-brand-muted transition-colors"
            />
          </div>
        </div>
            ...
            <>
              ...
              <div className="hidden md:block">
                <table className="w-full text-left text-sm">
                  <thead className="bg-brand-card2 border-b border-brand-border-subtle text-[#D9E4F5] text-xs sticky top-0 z-10 font-medium">
                    <tr>
                  ...
                  <tbody className="divide-y divide-brand-border-subtle">
                    {filteredProjects.map((p, i) => (
                      <tr key={p.id} onClick={() => navigate(`/project/${p.id}`)} className="hover:bg-[rgba(58,183,255,0.05)] transition-colors cursor-pointer group bg-transparent">
                      ...
                      <td className="px-5 py-4">
                        <span className="px-2.5 py-1 bg-[rgba(58,183,255,0.10)] border border-[rgba(58,183,255,0.22)] rounded-full text-[10px] text-[#DCEBFF]">
                          {p.scenario}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1 text-[11px] text-gray-400"><Database className="w-3.5 h-3.5 text-brand-accent" /> -</div>
                        </div>
                      </td>
                ...
                {filteredProjects.map((p, i) => (
                  <div key={p.id} onClick={() => navigate(`/project/${p.id}`)} className="bg-brand-card p-4 rounded-xl border border-brand-border-subtle cursor-pointer hover:border-brand-cyan/50 transition-colors shadow-[0_8px_24px_rgba(0,0,0,0.18)]">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                       <span className="px-2.5 py-1 bg-[rgba(58,183,255,0.10)] border border-[rgba(58,183,255,0.22)] rounded-full text-[10px] text-[#DCEBFF]">{p.scenario}</span>
                       ...
                       <span className="flex items-center gap-1.5 text-[10px] text-brand-cyan px-2.5 py-1 bg-[rgba(58,183,255,0.10)] border border-[rgba(58,183,255,0.22)] rounded-full"><div className="w-1.5 h-1.5 rounded-full bg-brand-cyan"></div> 分析中</span>
                    </div>
