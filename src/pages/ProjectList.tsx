...
export default function ProjectList() {
  ...
  useEffect(() => {
    ...
    setLoading(false);
    console.log(
      "[AuditEye] ProjectList loaded demo projects from local mockData",
      localProjects.map((p: any) => ({ id: p.id, riskScore: p.riskScore })),
    );
  }, []);
  ...
  const createProject = async () => {
    if (systemMode === "demo-readonly") {
      return toast(
        "当前系统为只读模式，无法创建项目。请检查数据库与 Blob 配置是否初始化。",
        "error",
      );
    }
    ...
    if (files.length === 0)
      return toast("请至少上传一个 PDF、Word 或 TXT 文件", "warning");
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex flex-col items-center justify-center p-4">
          <div className="bg-white border border-[rgba(15,23,42,0.08)] rounded-lg shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 space-y-4 overflow-y-auto">
              <div>
                <label className="text-xs text-brand-muted mb-1.5 block">
                  项目名称 <span className="text-[#F43F5E]">*</span>
                </label>
                <input
                  autoFocus
                  type="text"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewProject({ ...newProject, scenario: e.target.value })
                  }
                  className="w-full bg-white border border-[rgba(15,23,42,0.14)] rounded px-3 py-2 text-sm focus:outline-none focus:border-brand-blue text-brand-primary"
                >
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1.5 block">
                  上传初始文档 <span className="text-red-500">*</span>
                </label>
                <div
                  onClick={() => fileInputRef.current?.click()}
                  className="w-full bg-brand-deep border border-dashed border-[rgba(15,23,42,0.14)] rounded p-4 text-center cursor-pointer hover:border-brand-blue/50 transition-colors"
                >
                  {isSubmitting ? (
                ...
                {files.length > 0 && (
                  <div className="mt-3 space-y-1 max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                    {files.map((f, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between bg-[#F7FAFC] border border-[rgba(15,23,42,0.14)] p-2 rounded text-xs"
                      >
                        <span className="truncate flex-1 text-brand-primary">
                          {f.name}
                        </span>
                        <button
                          disabled={isSubmitting}
                          onClick={() => removeFile(i)}
                          className="text-gray-500 hover:text-red-500 ml-2 disabled:opacity-50"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      </div>
            ...
            <div className="p-4 border-t border-[rgba(15,23,42,0.06)] flex justify-end gap-3 bg-[rgba(15,23,42,0.02)] shrink-0">
              <button
                disabled={isSubmitting}
                className="px-4 py-2 text-sm font-medium text-brand-muted hover:text-brand-primary"
                onClick={() => setShowModal(false)}
              >
                取消
              </button>
              <button
                disabled={
                  isSubmitting || !newProject.name.trim() || files.length === 0
                }
                onClick={createProject}
                className="px-5 py-2.5 bg-brand-blue hover:bg-brand-blue-hover text-white font-medium text-sm rounded-lg shadow-[0_8px_20px_rgba(0,94,184,0.16)] flex items-center gap-2 transition-all hover:-translate-y-0.5 hover:brightness-110 disabled:opacity-50 disabled:hover:translate-y-0"
              >
                  ...
                  <tbody className="divide-y divide-[rgba(15,23,42,0.06)]">
                    {filteredProjects.map((p, i) => (
                      <tr
                        ...
                        className="hover:bg-[rgba(0,94,184,0.03)] transition-colors cursor-pointer group bg-transparent"
                      >
                        <td className="px-5 py-4">
                          ...
                          <div className="text-[10px] font-mono text-brand-muted mt-0.5">
                            PRJ-{p.id.toString().padStart(4, "0")}
                          </div>
                        </td>
                        <td className="px-5 py-4">
                          ...
                          <span className="flex items-center gap-1.5 text-[11px]">
                            {isAdmin ? (
                              <>
                                ...
                                <span className="text-amber-500">审计全权</span>
                              </>
                            ) : (
                              <>
                                <User className="w-3.5 h-3.5 text-gray-400" />{" "}
                                <span className="text-gray-400">用户只读</span>
                              </>
                            )}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-4 text-right">
                          <div
                            className={`font-bold ${getRiskVisual(p.riskScore, p.riskLevel).color}`}
                          >
                            {p.riskScore != null
                              ? Math.round(Number(p.riskScore))
                              : "-"}{" "}
                            <span className="text-[10px] text-gray-500 font-normal">
                              /100
                            </span>
                            <div className="text-[10px] font-normal leading-none mt-1">
                              {getRiskVisual(p.riskScore, p.riskLevel).label}
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-right">
              ...
              <div className="block md:hidden space-y-3 pb-6">
                {filteredProjects.map((p, i) => (
                  <div
                    key={p.id}
                    onClick={() => navigate(`/project/${p.id}`)}
                    className="bg-white p-4 rounded-xl border border-[rgba(15,23,42,0.08)] cursor-pointer hover:border-brand-cyan/50 transition-colors shadow-[0_8px_24px_rgba(0,0,0,0.18)]"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <div className="font-semibold text-brand-primary text-sm mb-1">
                          {p.name}
                        </div>
                      ...
                      <div
                        className={`text-right font-bold text-lg leading-none ${getRiskVisual(p.riskScore, p.riskLevel).color}`}
                      >
                        {p.riskScore != null
                          ? Math.round(Number(p.riskScore))
                          : "-"}
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      <span className="px-2.5 py-1 bg-[rgba(0,94,184,0.08)] border border-[rgba(0,94,184,0.14)] rounded-full text-[10px] text-[#005EB8] font-medium">
                        {p.scenario}
                      </span>
                      {p.industryType && (
                        <span className="px-2.5 py-1 bg-brand-blue/10 border border-brand-blue/20 rounded-full text-[10px] text-brand-blue font-medium">
                          {indNameMap[p.industryType] || p.industryType}
                        </span>
                      )}
                      <span className="flex items-center gap-1.5 text-[10px] text-brand-blue px-2.5 py-1 bg-[rgba(0,94,184,0.08)] border border-[rgba(0,94,184,0.14)] rounded-full font-medium">
                    ...
                    <div className="flex items-center justify-between text-[11px] text-brand-muted pt-3 border-t border-[rgba(15,23,42,0.06)]">
                      <div className="flex gap-3">
                        <div className="flex items-center gap-1">
                          <FileText className="w-3.5 h-3.5" /> {p.docCount || 0}
                        </div>
                        <div className="flex items-center gap-1">
