...
export default function ToastContainer() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none">
      {toasts.map(t => (
        <div key={t.id} className="animate-in slide-in-from-right-8 fade-in flex items-center gap-2 px-4 py-3 bg-[#0B2454] border border-[#1E4D92] shadow-lg rounded pointer-events-auto text-xs font-medium text-gray-200 min-w-[240px]">
          {t.type === 'success' && <CheckCircle className="w-4 h-4 text-green-500" />}
          {t.type === 'primary' && <Info className="w-4 h-4 text-[#0091DA]" />}
